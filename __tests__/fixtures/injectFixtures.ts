...
export function injectCurrentGameFixture(db: any) {
  db.prepare('DELETE FROM current_game').run();
  for (const current of currentGameFixture) {
    db.prepare(`INSERT INTO current_game (
        id, game_data, created_at, updated_at) VALUES (?, ?, ?, ?)`)
      .run(
        current.id,
        Array.isArray(current.game_data) || typeof current.game_data === 'object' ? JSON.stringify(current.game_data) : current.game_data,
        current.created_at,
        current.updated_at
      );
  }
...
export function injectPlayersFixture(db: any) {
  ...
  for (const player of playersFixture) {
    db.prepare('INSERT INTO players (player_id, player_name, created_at) VALUES (?, ?, ?)')
      .run(
        Array.isArray(player.player_id) || typeof player.player_id === 'object' ? JSON.stringify(player.player_id) : player.player_id,
        Array.isArray(player.player_name) || typeof player.player_name === 'object' ? JSON.stringify(player.player_name) : player.player_name,
        Array.isArray(player.created_at) || typeof player.created_at === 'object' ? JSON.stringify(player.created_at) : player.created_at
      );
  }
}
  ...
  for (const game of gamesFixture) {
    db.prepare(`INSERT INTO games (
      ...
      .run(
        ...
        game.game_image,
        typeof game.has_characters === 'boolean' ? (game.has_characters ? 1 : 0) : game.has_characters,
        Array.isArray(game.characters) || typeof game.characters === 'object' ? JSON.stringify(game.characters) : game.characters,
        game.min_players,
        game.max_players,
        typeof game.supports_cooperative === 'boolean' ? (game.supports_cooperative ? 1 : 0) : game.supports_cooperative,
        typeof game.supports_competitive === 'boolean' ? (game.supports_competitive ? 1 : 0) : game.supports_competitive,
        typeof game.supports_campaign === 'boolean' ? (game.supports_campaign ? 1 : 0) : game.supports_campaign,
        Array.isArray(game.default_mode) || typeof game.default_mode === 'object' ? JSON.stringify(game.default_mode) : game.default_mode,
        game.created_at
      );
  }
}
export function injectGameCharactersFixture(db: any) {
  db.prepare('DELETE FROM game_characters').run();
  for (const character of gameCharactersFixture) {
    db.prepare(`INSERT INTO game_characters (
...
export function injectGameExtensionsFixture(db: any) {
  db.prepare('DELETE FROM game_extensions').run();
  for (const ext of gameExtensionsFixture) {
    db.prepare(`INSERT INTO game_extensions (
      ...
      .run(
        ...
        ext.extensions_name,
        ext.base_game_id,
        Array.isArray(ext.extensions_description) || typeof ext.extensions_description === 'object' ? JSON.stringify(ext.extensions_description) : ext.extensions_description,
        ext.add_max_players,
        ext.created_at
      );
  }
...
export function injectGameSessionsFixture(db: any) {
  ...
  for (const session of gameSessionsFixture) {
    db.prepare(`INSERT INTO game_sessions (
      ...
      .run(
        ...
        session.game_mode,
        Array.isArray(session.sessions_players) ? JSON.stringify(session.sessions_players) : session.sessions_players,
        Array.isArray(session.sessions_scores) ? JSON.stringify(session.sessions_scores) : session.sessions_scores,
        Array.isArray(session.sessions_characters) ? JSON.stringify(session.sessions_characters) : session.sessions_characters,
        Array.isArray(session.sessions_extensions) ? JSON.stringify(session.sessions_extensions) : session.sessions_extensions,
        ...
        Array.isArray(session.sessions_dead_characters) ? JSON.stringify(session.sessions_dead_characters) : session.sessions_dead_characters,
        Array.isArray(session.sessions_new_character_names) ? JSON.stringify(session.sessions_new_character_names) : session.sessions_new_character_names,
        Array.isArray(session.sessions_character_history) ? JSON.stringify(session.sessions_character_history) : session.sessions_character_history,
        session.created_at
      );
  }
}
export function injectGameStatsFixture(db: any) {
...
export function injectPlayerStatsFixture(db: any) {
  db.prepare('DELETE FROM player_stats').run();
  for (const stat of playerStatsFixture) {
    db.prepare(`INSERT INTO player_stats (
      ...
      .run(
        Array.isArray(stat.stat_id) || typeof stat.stat_id === 'object' ? JSON.stringify(stat.stat_id) : stat.stat_id,
        Array.isArray(stat.player_id) || typeof stat.player_id === 'object' ? JSON.stringify(stat.player_id) : stat.player_id,
        Array.isArray(stat.total_games_played) || typeof stat.total_games_played === 'object' ? JSON.stringify(stat.total_games_played) : stat.total_games_played,
        Array.isArray(stat.total_wins) || typeof stat.total_wins === 'object' ? JSON.stringify(stat.total_wins) : stat.total_wins,
        Array.isArray(stat.total_losses) || typeof stat.total_losses === 'object' ? JSON.stringify(stat.total_losses) : stat.total_losses,
        Array.isArray(stat.total_score) || typeof stat.total_score === 'object' ? JSON.stringify(stat.total_score) : stat.total_score,
        Array.isArray(stat.average_score) || typeof stat.average_score === 'object' ? JSON.stringify(stat.average_score) : stat.average_score,
        Array.isArray(stat.last_game_date) || typeof stat.last_game_date === 'object' ? JSON.stringify(stat.last_game_date) : stat.last_game_date,
        Array.isArray(stat.created_at) || typeof stat.created_at === 'object' ? JSON.stringify(stat.created_at) : stat.created_at
      );
  }
}
export function injectPlayerGameStatsFixture(db: any) {
  db.prepare('DELETE FROM player_game_stats').run();
